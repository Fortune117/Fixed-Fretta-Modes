...
function ENT:Initialize()
	...
	self.Entity:CapabilitiesAdd( CAP_INNATE_MELEE_ATTACK1 ) 
	self.Entity:CapabilitiesAdd( CAP_MOVE_GROUND ) 
	
	--self.Entity:SetMaxYawRate( 5000 )
...
function ENT:SpawnRagdoll( att )
	...
	if IsValid( att ) then
		...
		local dir = ( att:GetPos() - self:GetPos() ):GetNormalized():Angle()
...
function ENT:Think()
	...
	if self.AttackTime and self.AttackTime < CurTime() then
		...
		if enemy and IsValid( enemy ) and enemy:GetPos():Distance( self.Entity:GetPos() ) < 64 then
...
function ENT:GetDoors()
	...
	table.insert( tbl, ents.FindByClass( "func_breakable" ) )
	table.insert( tbl, ents.FindByClass( "func_door" ) )
	table.insert( tbl, ents.FindByClass( "func_door_rotating" ) )
	table.insert( tbl, ents.FindByModel( "models/props_debris/wood_chunk03b.mdl" ) )
...
function ENT:NearDoor()
	...
	for k,v in pairs( doors ) do
		for k2,v2 in pairs( v ) do
			if IsValid( v2 ) then 
				if v2:GetPos():Distance( pos ) < 80 then
					return v2
				end
			end
		end 
