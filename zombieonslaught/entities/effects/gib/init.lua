...
function EFFECT:Init( data )
	...
	if phys and phys:IsValid() then
		...
		phys:AddAngleVelocity( Angle( math.random(-250,250), math.random(-250,250), math.random(-250,250) ):Forward() )
...
function EFFECT:Think()
	...
	if self.SoundTime and self.SoundTime < CurTime() then
		...
		sound.Play( table.Random( self.Sounds ), self:GetPos(), 80, math.random(90,110) )
		self.SoundTime = nil
