...
function SWEP:PrimaryAttack()
	...
	if( ( ( CLIENT && IsFirstTimePredicted() ) || SERVER ) and IsValid( self.Owner ) ) then
...
function SWEP:Think()
	...
	if( CurTime() >= (self.NextIdleTime or 0) and self.PlayIdleAnim ) then
		self:IdleThink();
	end

	local que = self.BurstQueue or 0 
	if que > 0 && CurTime() >= que then
		...
		if( SERVER ) then
			if( b == true ) then
				if( self.IronsightDelayFOV ) then
					...
					timer.Simple( delay, function() self:SetFOV( self.IronsightsFOV, 0, true ) end );
				else
